...
class Billing extends Backend_Controller
{
	...
	public function payment_methods($method_type = NULL)
	{
		switch ($method_type) {
			case "direct_bank_transfer":
				$this->direct_bank_transfer($method_type);
				break;
			case "cash_on_delivery":
				$this->cash_on_delivery($method_type);
				break;
			default:
	...
	private function payment_method_list()
	{
		...
		else
		{
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			//echo '<pre>'; print_r($this->data); echo '</pre>'; die();
			$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'billing' . DIRECTORY_SEPARATOR . 'index', $this->data);
		}
	}

	private function direct_bank_transfer($method_type)
	{
		if (!$this->ion_auth->logged_in())
		{
			// redirect them to the login page
		...
		else
		{
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			//echo '<pre>'; print_r($this->data); echo '</pre>'; die();
			$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'billing' . DIRECTORY_SEPARATOR . $method_type, $this->data);
		}
	}
	private function cash_on_delivery($method_type)
	{
		if (!$this->ion_auth->logged_in())
		{
			// redirect them to the login page
