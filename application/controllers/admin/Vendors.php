...
class Vendors extends Backend_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'vendors' . DIRECTORY_SEPARATOR . 'index', $this->data);
		}
	}
	...
	public function create_vendor()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			$groups = array(4);
			$vendor_id = $this->ion_auth->register($identity, $password, $email, $additional_data, $groups);
			$user_metadata = array(
				array(
					'user_id' => $vendor_id,
			...
			$this->warehouses_model->add_warehouse($warehouse_data);
		...
		$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'vendors' . DIRECTORY_SEPARATOR . 'create_vendor', $this->data);
	}
	...
	public function edit_vendor($id)
	{
		...
		$this->data['message'] = (validation_errors() ? validation_errors() : ($this->ion_auth->errors() ? $this->ion_auth->errors() : $this->session->flashdata('message')));
		$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'vendors' . DIRECTORY_SEPARATOR . 'edit_vendor', $this->data);
	}
