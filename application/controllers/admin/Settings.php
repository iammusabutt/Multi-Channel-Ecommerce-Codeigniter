...
class Settings extends Backend_Controller
{
	...
	public function developer()
	{
		...
		else
		{
			if($_POST)
			{
				$meta_table = $this->input->post('meta_table');
				$object_type = $this->input->post('data_type');
				$meta_key = $this->input->post('meta_key');
				$meta_value = $this->input->post('meta_value');
			
				// validate form input
				$this->form_validation->set_rules('meta_table','Meta Table','trim|required');
				...
				$this->form_validation->set_rules('meta_value','Meta Value','trim');

				if ($this->form_validation->run() === TRUE)
				{
					$objects = $this->products_model->get_objects($object_type, $parent = NULL, $author = NULL, $status = 'publish');
					$object_ids = array_column($objects, 'object_id');
					foreach($object_ids as $object_id) {
						$objectmeta = $this->settings_model->get_objectmeta_record($object_id, $meta_key, $meta_table);
						if(empty($objectmeta)){
							$metaarray['object_id'] = $object_id;
							$metaarray['meta_key'] = $meta_key;
							$metaarray['meta_value'] = $meta_value;
							$object_id_array[] = $metaarray;
						}
					}
					$this->settings_model->add_objectmeta($object_id_array);
					redirect('admin/settings/developer', 'refresh');
				}
			}
			$this->data['data_type'] = array(
				'name' => 'data_type',
				'id' => 'data_type',
				'type' => 'text',
				'value' => $this->form_validation->set_value('data_type'),
				'class' => 'form-control',
				'placeholder' => '',
			);
			$this->data['meta_table'] = array(
				'name' => 'meta_table',
				'id' => 'meta_table',
				'type' => 'text',
				'value' => $this->form_validation->set_value('meta_table', 'objectmeta'),
				'class' => 'form-control',
				'placeholder' => '',
			);
			$this->data['meta_key'] = array(
				...
				'id' => 'meta_key',
				'type' => 'text',
				'value' => $this->form_validation->set_value('meta_key'),
				'class' => 'form-control',
				'placeholder' => '',
			);
			$this->data['meta_value'] = array(
				'name' => 'meta_value',
				'id' => 'meta_value',
				'type' => 'text',
				'value' => $this->form_validation->set_value('meta_value'),
				'class' => 'form-control',
				'placeholder' => '',
			);
			// set the flash data error message if there is one
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render_page('admin' . DIRECTORY_SEPARATOR . 'settings' . DIRECTORY_SEPARATOR . 'developer', $this->data);
		}
	}
	public function ajax_add_product_metadata()
	{
		if (!$this->ion_auth->logged_in())
		{
			// redirect them to the login page
		...
		else
		{
			...
			if(!empty($object_id_array)){
				$response = array('response'=>'yes', 'message'=>'This meta key is now linked to all the products successfully.');
			} else {
				$response = array('response'=>'no', 'message'=>'All the products are already linked to this meta key.');
			}
			$this->output->set_content_type('application/json');
			$this->output->set_output(json_encode($response));
		}
	}
	public function ajax_delete_product_metadata()
	{
		if (!$this->ion_auth->logged_in())
		{
			// redirect them to the login page
		...
		else
		{
			if($_POST)
			{
				...
				if ($this->form_validation->run() === TRUE)
				{
					$this->settings_model->delete_objectmeta_by_meta_key($meta_key, $meta_table);
				}
			}
			$response = array('response'=>'yes', 'message'=>'This meta key is unlinked from all the products successfully.');
			$this->output->set_content_type('application/json');
			$this->output->set_output(json_encode($response));
		}
	}
