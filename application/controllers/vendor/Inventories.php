 ...
 */
class Inventories extends Vendor_Controller
{
	...
	{	
		...
		$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
		$this->_render_page($this->access_type . DIRECTORY_SEPARATOR . 'inventories' . DIRECTORY_SEPARATOR . 'index', $this->data);
	}
	...
	private function product_variation_edit($object_id, $variation_id)
	{
		...
		else
		{
			...
			if(!empty($variation))
			{
				...
				//echo '<pre>'; print_r($warehouse_default); echo '</pre>';die();
	...
	public function ajax_compute_inventory()
	{
		...
		else
		{
			...
			if($_POST)
			{
				...
				if ($this->form_validation->run() === TRUE)
				{
					//echo '<pre>'; print_r($variable_stock_array); echo '</pre>';
