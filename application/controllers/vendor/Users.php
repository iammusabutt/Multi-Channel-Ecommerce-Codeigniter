 ...
 */
class Users extends Vendor_Controller
{
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			//echo '<pre>'; print_r($user_metadata); echo '</pre>'; die();
			$this->users_model->add_member_metadata($user_metadata);
			$this->session->set_flashdata('message', $this->ion_auth->messages());
			redirect("member/members", 'refresh');
		}
		...
		$this->_render_page('member' . DIRECTORY_SEPARATOR . 'users' . DIRECTORY_SEPARATOR . 'create_member', $this->data);
	}
	...
	public function edit_user($id)
	{
		...
		if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				$usermeta = $this->users_model->get_member_metadata($id);
				$meta = array();
				...
				$this->users_model->update_member_metadata($meta);
				// update the password if it was posted
				if ($this->input->post('password'))
				{
				...
				if ($this->ion_auth->update($user->id, $data))
				{
					...
					$this->session->set_flashdata('message', $this->ion_auth->messages());
					redirect('member/members/edit_member/'.$id, 'refresh');
		...
		$member = $this->users_model->get_member_by_id($id);
		$member_metadata = $this->users_model->get_member_metadata($id);
		if(!empty($member_metadata)){
			foreach ($member_metadata as $key => $item) {
				...
				if($item['meta_key'] == 'marketplace_id')
				{
					...
					$itemArr['marketplace_name'] = $currency['marketplace_name'];
					//echo '<pre>'; print_r($currency); echo '</pre>'; die();
				}
