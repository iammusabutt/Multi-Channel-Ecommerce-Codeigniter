...
class Orders extends Member_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			$orders = $this->users_model->get_users_orders_by_member($member_id, $from_date, $to_date);
			//echo '<pre>'; print_r($orders); echo '</pre>'; die();
			if(!empty($orders)){
	...
	public function add_tracking($order_id)
	{
		...
		else
		{
			if($_POST)
			{
				...
				if ($this->form_validation->run() === TRUE)
				{
					$orders_meta = $this->orders_model->get_ordermeta($order_id);
					$ordermeta = array();
					foreach ($orders_meta as $key => $order){
					...
					//echo '<pre>'; print_r($ordermeta); echo '</pre>'; die();
					$this->orders_model->update_ordermeta($order_id, $ordermeta);
