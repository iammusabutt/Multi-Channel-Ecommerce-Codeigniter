<!-- Start content -->
	<div class="content">
		...
		<!-- Page-Title -->
			...
			<?php echo form_open(uri_string() . "?post_type=product&&product_id=". $_GET['product_id'] ."&&action=edit", $attributes);?>
			...
			<div class="row">
					...
					<div class="card-box product_data_addon">
						<div class="row">
							...
							<div class="col-3">
								<select name="product_type" id="product_type" class="form-control selectpicker">
									...
									<option value="simple" <?php if($term_relation['name'] == 'simple'){echo 'selected';}else{echo '';};?>>Simple Product</option>
									<option value="variable" <?php if($term_relation['name'] == 'variable'){echo 'selected';}else{echo '';};?>>Variable Product</option> 
									</optgroup>
								</select>
							</div>
						</div>
						...
						<div class="tabs-vertical-env">
							<ul class="nav tabs-vertical" id="variable_tabs">
								...
								<li class="nav-item simple <?php if($term_relation['name'] == 'variable'){echo 'visible';}else{echo '';};?>" id="simple">
							...
							<div class="tab-content">
								...
								<div class="tab-pane" id="v-attributes">
									<div class="row">
										...
										<div class="col-6">
											<a href="#" id="select_attribute" data-object-id="<?php echo $_GET['product_id'];?>" class="btn btn-gold waves-effect waves-light">Add</a>
										</div>
									</div>
									<hr>
												...
												<span href="#<?php echo $block['attribute_slug'];?>" class="text-dark anchor-arch" data-toggle="collapse" aria-expanded="true" aria-controls="<?php echo $block['attribute_slug'];?>">
													<h6 class="m-0">
														<small class="text-muted">
															<a id="delete_attribute" data-object-id="<?php echo $_GET['product_id'];?>" data-attributevalue="<?php echo $block['attribute_slug'];?>" class="remove_row delete text-danger">Remove</a>
														</small>
									...
									<div class="row">
										<div class="col-12">
											<a id="save_attribute" data-object-id="<?php echo $_GET['product_id'];?>" class="btn btn-gold waves-effect waves-light">Save attributes</a>
										</div>
									</div>
								</div>
								<div class="tab-pane" id="v-variations">
									<div class="row">
										...
										<div class="col-6">
											<a id="add_variation" data-object-id="<?php echo $_GET['product_id'];?>" class="btn btn-gold waves-effect waves-light">Go</a>
										</div>
									</div>
									<hr>
											...
											<div class="card-header single-row p-0" id="heading<?php echo $variation['object_id'];?>">
												...
												<?php endif;?>
													<h6 class="m-0 var-delete-position">
														<small class="text-muted">
															<a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/products?post_type=product_variation&&product_id=<?php echo $_GET['product_id'];?>&&action=edit&&variation_id=<?php echo $variation['object_id'];?>" class="btn btn-gold waves-effect waves-light variation_edit">Edit</a> 
															<a id="delete_variation" data-object-id="<?php echo $_GET['product_id'];?>" data-variation-id="<?php echo $variation['object_id'];?>" class="remove_row delete text-danger variation_row">Remove</a> 
														</small>
													</h6>
												</span>
											</div>
									...
									<div class="row">
										<div class="col-12">
											<a id="save_variation" data-object-id="<?php echo $_GET['product_id'];?>" class="btn btn-gold waves-effect waves-light">Save variation</a>
										</div>
									</div>
								</div>
							</div>
				...
				<div class="col-lg-3 col-md-3 col-sm-12">
					<div class="card-box">
						<div class="row">
							<div class="col-12">
								<a class="btn btn-gold btn-block waves-effect waves-light" onclick="document.addgamer.submit()">Update Product</a>
							</div>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-12">
								...
								<div class="mt-3">
									...
									<?php if(!empty($term['term_taxonomy_id']) && $term['object_id'] == $_GET['product_id']):?>
						...
						<div class="portlet-heading bg-gold p-t-0 p-b-0">
							<div class="portlet-widgets portlet-header-sm">
								<a data-toggle="collapse" data-parent="#accordion1" href="#bg-primary">
									<h6 class="pull-left text-white m-0 m-r-10 l-h-34">Product image</h6>
						...
						<div id="bg-primary" class="panel-collapse collapse show">
							<div class="portlet-body">
								...
								<?php if(!empty($product_image->thumb)){;?>
									<input type="file" class="dropify" name="file" data-height="120" data-default-file="<?php echo base_url().$product_image->thumb;?>" data-allowed-file-extensions="jpeg jpg png"  data-show-remove="false">
								<?php } else {;?>
									<input type="file" class="dropify" name="file" data-height="120" data-allowed-file-extensions="jpeg jpg png"  data-show-remove="false">
								<?php } ;?>
									<button id="uploadify" data-image-type="product-image" class="btn btn-default btn-block waves-effect waves-light m-t-10">Upload</button>
								</div>
							</div>
						</div>
					</div>
						...
						<div id="bg-gallery" class="panel-collapse collapse show">
							<div class="portlet-body">
								...
								<?php } else {;?>
									<input type="file" class="dropify-gallery" name="file" data-height="120" data-allowed-file-extensions="jpeg jpg png"  data-show-remove="false">
								<?php } ;?>
									<button id="uploadify" data-image-type="product-gallery" class="btn btn-default btn-block waves-effect waves-light m-t-10">Upload</button>
									<hr>
									<div class="row">
	...
	$('#variable_tabs a.variations').on('shown.bs.tab', function (e) {
    	...
    	e.stopPropagation();
    	var object_id = "<?php echo $_GET['product_id'];?>";
