<!-- Start content -->
	<div class="content">
		...
		<!-- Page-Title -->
			...
			<?php echo form_open(uri_string().'?post_type=product_variation&&product_id='.$product['object_id'].'&&action=edit&&variation_id='.$variation['object_id'], $attributes);?>
				...
				<div class="col-lg-3 col-md-3 col-sm-12">
					<div class="card m-b-20 card-body">
						<div class="row">
							...
							<hr>
							<div class="col-md-4">
								<img src="<?php echo base_url();?>uploads/images/products/thumb/64f2b0c4d0d8253e2f9835e067389fdc.jpg" alt="image" class="img-fluid thumb-md">
							</div>
							<div class="col-md-8 p-l-0">
								<p><?php echo $product['object_title'];?></p>
								<p class="m-0"><a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/products?post_type=product&&product_id=<?php echo $product['object_id'];?>&&action=edit" class="text-primary pointer font-12"><i class="ti-back-right"></i> back to parent</a></p>
							</div>
						</div>
					</div>
					<div class="portlet variants-section">
						<div class="portlet-heading bg-gold p-t-0 p-b-0">
							<div class="portlet-widgets portlet-header-sm">
								<a data-toggle="collapse" data-parent="#accordion1" href="#bg-primary">
									<h6 class="pull-left text-white m-0 m-r-10 l-h-34">Options</h6>
						...
						<div id="bg-primary" class="panel-collapse collapse show">
							<div class="portlet-body">
								<div class="m-b-0">
									<div class="col-md-12">
										<?php $checked = $variation['manage_stock'] == 'yes' ? 'checked' : '';?>
										<div class="custom-control custom-checkbox">
											<input type="hidden" name="manage_stock" value="no">
											<input type="checkbox" class="custom-control-input" id="customCheck1" name="manage_stock" value="yes" <?php echo $checked;?>>
											<label class="custom-control-label pointer" for="customCheck1">Managed Stock</label>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="portlet variants-section">
						<div class="portlet-heading bg-gold p-t-0 p-b-0">
							<div class="portlet-widgets portlet-header-sm">
								<a data-toggle="collapse" data-parent="#accordion1" href="#bg-secondary">
						...
						<div id="bg-secondary" class="panel-collapse collapse show">
							<div class="portlet-body">
								<div class="m-b-0">
									<ul class="list-group list-group-flush">
											<li class="list-group-item">
												<p class="p-20 p-t-0 p-b-0"><a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/products?post_type=product&&product_id=<?php echo $product['object_id'];?>&&action=variations" class="text-primary pointer font-12"><i class="ti-back-right"></i> back to variations</a></p>	
											</li>
										<?php if(!empty($product['variations'])): foreach ($product['variations'] as $key => $value):?>
											<li class="list-group-item">
												<a href="<?php echo base_url();?><?php echo $this->uri->segment(1);?>/products?post_type=product_variation&&product_id=<?php echo $product['object_id'];?>&&action=edit&&variation_id=<?php echo $value['object_id'];?>" class="waves-effect btn-block variant-btn <?php if($_GET['variation_id'] == $value['object_id']){ echo 'active';};?>"><?php foreach ($value['attributes'] as $attribute):?><?php echo $attribute;?> <?php endforeach;?></a>	
											</li>
										<?php endforeach; else:?>
										<?php endif;?>
									</ul>
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.response == "yes") {
					l.stop();
         	 		$('.additional-warehouse-locations').html(data.content);
					$.Notification.autoHideNotify('success', 'top right', 'Warehouse Added!', data.message);
				} else {
					l.stop();
					$.Notification.autoHideNotify('error', 'top right', 'Something went wrong', data.message);
				}
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.response == "yes") {
					l.stop();
         	 		$('.additional-warehouse-locations').html(data.content);
					$.Notification.autoHideNotify('error', 'top right', 'Warehouse Deleted!', data.message);
				} else {
					l.stop();
					$.Notification.autoHideNotify('error', 'top right', 'Something went wrong!', data.message);
				}
